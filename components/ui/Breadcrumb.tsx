        ...
        {mobileItems.map(({ name, item }, index, arr) => {
          ...
          return (
            <li class="gap-2" key={item}>
              {isLast
                ? (
                  <span class="font-semibold font-Hanken-Grotesk text-xs text-black-10">
                    {name}
                  </span>
                )
                : (
                  <a
                    class="text-xs font-Hanken-Grotesk text-gray-35 no-underline"
                    href={relative(item)}
                  >
                    {name}
                  </a>
        ...
        {items.map(({ name, item }, index, arr) => {
          ...
          return (
            <li key={item}>
              {isLast
                ? (
                  <span class="font-Hanken-Grotesk font-semibold text-xs text-black-10">
                    {name}
                  </span>
                )
                : (
                  <a
                    class="font-Hanken-Grotesk text-xs text-gray-35 no-underline"
                    href={relative(item)}
                  >
                    {name}
                  </a>
