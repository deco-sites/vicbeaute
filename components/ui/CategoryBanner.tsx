...
function BreadcrumbInsideBanner({ pathname, darkText }: { pathname: string, darkText?: boolean }) {
  ...
  return (
    <ul class={`flex items-center gap-[6px] text-[13px] font-medium tracking-wide relative z-20 ${textColor}`}>
      <li>
        <a href="/" class="hover:underline transition-all drop-shadow-md">Inicio</a>
      </li>
      {parts.map((p, i) => {
         ...
         return (
            <li class="flex items-center gap-[6px]" key={href}>
               <span class={`opacity-90 text-[10px] mt-[1px] drop-shadow-md ${textColor}`}>/</span>
               {isLast ? (
                  <span class="opacity-100 font-semibold drop-shadow-md">{name}</span>
               ) : (
                  <a href={href} class="hover:underline opacity-90 transition-opacity drop-shadow-md">{name}</a>
               )}
            </li>
         )
      })}
...
function CategoryBanner(props: SectionProps<ReturnType<typeof loader>>) {
  const { 
    ...
    imageDesk = "https://ozksgdmyrqcxcwhnbepg.supabase.co/storage/v1/object/public/assets/239/ec597b6a-dcf1-48ca-a99d-95b3c6304f96", 
    imageMobile = "https://ozksgdmyrqcxcwhnbepg.supabase.co/storage/v1/object/public/assets/239/91102b71-4832-486a-b683-5f7b06f649af", 
    imageAlt = "", 
    pathname = "", 
    darkText 
  } = props;
  ...
  return (
    <div class="relative w-full h-[300px] lg:h-[360px] overflow-hidden bg-black flex items-center">
      ...
      <div class="relative w-full h-full z-10 max-w-[1440px] mx-auto">
         ...
         <h1 class={`absolute left-[12px] lg:left-[80px] top-[50%] -translate-y-[40%] lg:-translate-y-1/2 font-Queens text-[50px] lg:text-[76px] leading-[1] drop-shadow-lg ${h1Color}`}>
           {title}
         </h1>
