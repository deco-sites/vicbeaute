      ...
      <div id={id} class="relative flex flex-col w-full">
        ...
        <div class="hidden lg:grid grid-cols-2 gap-2 w-full">
          {images.map((img, index) => (
            <div
              key={index}
              class="relative w-full aspect-square overflow-hidden bg-[#F8F8F8]"
            >
              {/* Ajuste de Image Source pra alta resolução */}
              {index === 0 && (
                <div class="absolute top-4 left-4 z-10 flex flex-col gap-2">
                  <span class="bg-[#e4e2cd] text-[#191C1F] text-[9px] font-bold px-2 py-1 uppercase tracking-wider">
                    Hit no TikTok
                  </span>
                  <span class="bg-[#3e3e3b] text-white text-[9px] font-bold px-2 py-1 uppercase tracking-wider">
                    Mais Vendidos
                  </span>
                </div>
              )}
        ...
        <div class="lg:hidden relative w-full flex-grow">
          ...
          {/* Seta Voltar Mobile */}
          <Slider.PrevButton class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center rounded-full shadow-sm bg-white-15 transition-colors disabled:opacity-30 disabled:cursor-not-allowed z-10">
            <Icon
              id="chevron-right"
              width={9}
              height={18}
              class="rotate-180 text-black"
              strokeWidth={2}
            />
          </Slider.PrevButton>
