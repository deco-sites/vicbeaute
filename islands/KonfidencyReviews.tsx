...
function StarIcon(
  { filled, size = 16, borderOnly = false }: {
    filled: boolean;
    size?: number;
    borderOnly?: boolean;
  },
...
export default function KonfidencyReviews(
  { reviews = [], aggregateRating }: Props,
) {
  ...
  return (
    <div class="w-full bg-white text-[#4C4C4C] font-Hanken-Grotesk [&_strong]:font-normal [&_b]:font-normal">
      {/* Header Topic */}
      ...
      <div class="container max-w-[1044px] px-5 lg:px-0 mx-auto flex flex-col pt-0 pb-12 lg:pb-16 gap-10">
        ...
        <div class="flex flex-col lg:flex-row justify-between lg:items-center py-6 border-b border-t border-[#E6E6E6] gap-8">
          ...
          <div class="flex flex-row items-center gap-4">
            <span class="text-[32px] lg:text-[48px] font-semibold leading-none">
              {totalRating.toFixed(1).replace(".", ",")}
            </span>
            <div class="flex flex-col">
              <div class="flex flex-row gap-1 [&_strong]:flex">
                {[1, 2, 3, 4, 5].map((s) => (
                  <StarIcon
                    key={s}
                    filled={s <= Math.round(totalRating)}
                    size={20}
                  />
                ))}
              </div>
          ...
          <div class="flex flex-col min-w-[200px] lg:min-w-[300px] gap-3">
            <div class="flex justify-between items-center text-sm gap-4">
              <span class="w-32 lg:w-40 text-left">
                Facilidade de aplicação
              </span>
              <div class="flex-1 bg-[#EEEDEA] h-1.5 rounded-full overflow-hidden">
                <div
                  class="bg-[#ACAD9C] h-full"
                  style={{ width: easeOfApplicationPercent }}
                >
                </div>
              </div>
              <span class="w-8 text-right bg-transparent">
                {easeOfApplicationDisplayParam}
              </span>
            </div>
          ...
          <div class="flex justify-start lg:justify-end">
            <button class="bg-[#546351] text-white-15 hover:bg-[#435240] transition-colors py-[10px] px-8 text-sm uppercase font-semibold cursor-pointer tracking-widest">
              Quero avaliar
            </button>
          </div>
        </div>
        ...
        {/* Reviews List Header */}
        <div class="flex flex-row justify-between items-start lg:items-center -mb-2 mt-4 gap-4 [&_strong]:contents">
          <span class="text-[22px] lg:text-[24px] font-semibold text-black">
            {reviewCount} avaliações
          </span>
          <div class="flex items-center text-sm text-gray-500">
            <div class="relative w-[120px] h-[34px] bg-[#EFEFEF] border border-[#E8E8E8] rounded-[3px] flex items-center cursor-pointer">
              <select class="w-full h-full appearance-none bg-transparent outline-none font-Hanken-Grotesk text-[14px] text-[#4C4C4C] px-[15px] cursor-pointer z-10">
              ...
              <svg
                class="absolute right-[15px] w-3.5 h-3.5 text-[#546351] pointer-events-none z-0"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
          ...
          {reviews.length === 0
            ...
            : (
              reviews.map((rv, index) => {
                ...
                const dateObj = rv.datePublished
                  ? new Date(rv.datePublished)
                  : null;
                ...
                const images: string[] = Array.isArray(rv.image)
                  ? rv.image.map((i) =>
                ...
                return (
                  <div
                    key={index}
                    class="flex flex-col border-b border-[#E6E6E6] pb-8 gap-4"
                  >
                    {/* Card Header */}
                    <div class="flex flex-col lg:flex-row justify-between lg:items-start w-full">
                      <div class="flex flex-col gap-2">
                        <div class="flex gap-1 [&_strong]:flex">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <StarIcon
                              key={s}
                              filled={s <= Math.round(rvRating)}
                              size={14}
                            />
                          ))}
                        </div>
                    ...
                    <div class="flex flex-col lg:flex-row w-full gap-4 lg:gap-10 xl:items-start">
                      {/* Text & Images Box */}
                      <div class="flex-1 flex flex-col gap-3">
                        <p class="text-[14px] leading-relaxed text-[#333333] [&_strong]:font-normal">
                          {rv.reviewBody}
                        </p>
                        {images.length > 0 && (
                          <div class="flex gap-2.5 overflow-x-auto mt-2">
                            {images.map((imgUrl, i) => (
                              <div
                                key={i}
                                class="w-[80px] h-[80px] flex-shrink-0 bg-gray-100 p-1"
                              >
                                <img
                                  src={imgUrl}
                      ...
                      <div class="flex flex-col justify-end items-start lg:items-end lg:w-48 pt-4 lg:pt-0 pb-1 shrink-0">
                        <span class="text-[10px] text-gray-500 uppercase font-semibold tracking-wider mb-2">
