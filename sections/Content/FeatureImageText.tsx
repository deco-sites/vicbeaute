...
export default function FeatureImageText({
  ...
  content = {
    ...
    descriptionMobile:
      "Desliza, esfuma fácil e não pesa.<br/>Não marca poros e seca confortável.",
  ...
  const descMobileClass =
    DESC_MOBILE_COLORS[content.descriptionMobileColor || "Cinza Escuro"];
    ...
    <div class="w-full bg-white-10 py-10 lg:py-20 lg:px-8 xl:px-0">
      <div class="max-w-[1140px] mx-auto flex flex-col lg:flex-row lg:bg-white-15 lg:rounded-2xl lg:overflow-hidden lg:shadow-sm">
        ...
        >
          {content.title && (
            <h2
              class={`font-Queens text-[36px] sm:text-[42px] lg:text-[48px] xl:text-[52px] ${titleClass} leading-[1.05] mb-4 lg:mb-6 text-center lg:text-left`}
            >
              {content.title}
            </h2>
          )}
          ...
          {content.descriptionDesktop && (
            <div
              class={`${
                content.descriptionMobile ? "hidden lg:block" : "block"
              } text-black-20 text-[15px] lg:text-[16px] leading-relaxed mb-8 text-center lg:text-left [&>p]:mb-4 last:[&>p]:mb-0`}
