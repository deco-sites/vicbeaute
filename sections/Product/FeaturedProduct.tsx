    ...
    try {
      ...
      if (baseProduct) {
        ...
        try {
          ...
          console.warn(
            "[FeaturedProduct] Aviso: Não foi possível carregar os similares.",
            err,
          );
        }
...
export default function FeaturedProduct({
  ...
  subtitle =
    "O Stick Tudo foi feito para facilitar sua rotina e valorizar sua beleza real.",
  page,
  customProductHook,
}: Props) {
  ...
  let allProducts: Product[] = similars.length > 0
    ? [product, ...similars]
  ...
  const allColors = allProducts
    ...
    .map((p) => {
      ...
      return {
        url: urlStr,
        name: p.additionalProperty?.find((a) => a.name === "Cor")?.value ??
          p.name ?? "",
        subtitle:
  ...
  const images = (product.image ?? []).filter((img) =>
  ...
  return (
    <div class="relative w-full py-10 lg:py-16">
      <div class="max-w-[1140px] mx-auto border border-[#B0D3BB] bg-white-15 rounded-[20px] overflow-hidden flex flex-col lg:flex-row p-6 lg:p-12 gap-8 lg:gap-14 shadow-sm items-center">
        ...
        <div
          class="w-full lg:w-1/2 relative bg-[#F9F7F5] rounded-xl flex items-center justify-center p-8 aspect-square"
          id="featured-buybox-img"
        >
            ...
            <>
              <div class="absolute inset-y-0 left-4 flex items-center justify-start z-10">
                <Slider.PrevButton class="btn btn-circle bg-white-15 border-white-15 shadow-md hover:bg-gray-50 flex items-center justify-center min-h-0 h-10 w-10 btn-sm disabled:opacity-30">
                  <Icon
                    class="text-black pointer-events-none rotate-180"
                    size={16}
                    id="chevron-right"
                    strokeWidth={3}
                  />
